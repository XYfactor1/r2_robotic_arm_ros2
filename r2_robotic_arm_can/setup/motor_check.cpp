...
void print_motor_status(const r2_robotic_arm::damiao_motor::Motor& motor) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        std::cout << "=== R2RoboticArm Motor Control Script ===" << std::endl;
        ...
        // Initialize R2RoboticArm with CAN interface
        std::cout << "Initializing R2RoboticArm CAN..." << std::endl;
        r2_robotic_arm::can::socket::R2RoboticArm r2_robotic_arm(can_interface,
                                              use_fd);  // Use specified interface and FD setting

        // Initialize single motor
        std::cout << "Initializing motor..." << std::endl;
        r2_robotic_arm.init_arm_motors({r2_robotic_arm::damiao_motor::MotorType::DM4310}, {send_can_id},
                                {recv_can_id});
        ...
        r2_robotic_arm.recv_all();
        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        r2_robotic_arm.query_param_all(static_cast<int>(r2_robotic_arm::damiao_motor::RID::can_br));
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        r2_robotic_arm.recv_all();
        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        r2_robotic_arm.query_param_all(static_cast<int>(r2_robotic_arm::damiao_motor::RID::CTRL_MODE));
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        r2_robotic_arm.recv_all();
        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        // Get motor and verify parameters
        const auto& motors = r2_robotic_arm.get_arm().get_motors();
        if (!motors.empty()) {
            ...
            double queried_baudrate =
                motor.get_param(static_cast<int>(r2_robotic_arm::damiao_motor::RID::can_br));
            double queried_control_mode =
                motor.get_param(static_cast<int>(r2_robotic_arm::damiao_motor::RID::CTRL_MODE));
            ...
            if (queried_control_mode != static_cast<int>(r2_robotic_arm::damiao_motor::ControlMode::MIT)) {
