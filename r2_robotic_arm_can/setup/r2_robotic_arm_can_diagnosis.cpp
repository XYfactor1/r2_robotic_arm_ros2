...
int main(int argc, char* argv[]) {
    std::cout << "R2RoboticArm CAN diagnostics\n";
    ...
    // Use the actual args
    r2_robotic_arm::can::socket::R2RoboticArm r2_robotic_arm(can_if, use_fd);

    // Initialize arm motors
    std::vector<r2_robotic_arm::damiao_motor::MotorType> motor_types = {
        r2_robotic_arm::damiao_motor::MotorType::DM8009, r2_robotic_arm::damiao_motor::MotorType::DM8009,
        r2_robotic_arm::damiao_motor::MotorType::DM4340, r2_robotic_arm::damiao_motor::MotorType::DM4340,
        r2_robotic_arm::damiao_motor::MotorType::DM4310, r2_robotic_arm::damiao_motor::MotorType::DM4310,
        r2_robotic_arm::damiao_motor::MotorType::DM4310};
    ...
    r2_robotic_arm.query_param_all((int)r2_robotic_arm::damiao_motor::RID::MST_ID);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    r2_robotic_arm.recv_all();
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    r2_robotic_arm.query_param_all((int)r2_robotic_arm::damiao_motor::RID::can_br);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    r2_robotic_arm.recv_all();
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // read params
    const auto& arm_motors = r2_robotic_arm.get_arm().get_motors();

    std::vector<uint32_t> missing_ids;

    for (size_t i = 0; i < arm_motors.size(); ++i) {
        const auto& motor = arm_motors[i];
        double mst = motor.get_param((int)r2_robotic_arm::damiao_motor::RID::MST_ID);
        double br = motor.get_param((int)r2_robotic_arm::damiao_motor::RID::can_br);
